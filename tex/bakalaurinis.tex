...
\textquote{Linkedin} duomenų aibėmis, parenka slaptažodžius kiekiais, pateiktais 
...
nėra.
Kadangi \textquote{PCFG} straipsniuose \cite{PCFG1,PCFG2} yra naudojami kitokios 
...
duomenų bazės, negali būti efektyviai palygintas su \textquote{PassGAN} autorių 
eksperimentais \cite{PassGAN}.
% TODO: Paimti kitą DB tik įrodyti, kad algoritmas veikia panašiai?
...
\begin{table}[hb]
  \centering
  \caption{
    Slaptažodžių, sugeneruotų \textquote{PassGAN} metodo modeliu, apmokytu 80 \% 
    \textquote{RockYou} duomenų, rezultatai testuojant su 20 \% 
    \textquote{RockYou} testavimo duomenų aibe.
  }
...
\end{table}
Apmokytas modelis su 80 \% \textquote{RockYou} duomenų rinkiniu ir, pirmiausia, 
...
yra priskiriamas nutekintos slaptažodžių duomenų bazės \textquote{RockYou} 
...
\begin{table}[hb]
  \centering
  \caption{
    Slaptažodžių, sugeneruotų \textquote{PassGAN} metodu modelio, apmokytu 80 \% 
    \textquote{RockYou} duomenų, sugeneruotų slaptažodžių testavimas su visa
    \textquote{Linked} testavimo duomenų aibe.
  }
  \begin{tabular}{|c|c|c|c|}
    \hline \textbf{Kiekis} & \textbf{Unikalūs} & \textbf{Sutampantys} & 
    \textbf{Sutampantys (\%)} \\
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{
    Sutampančių su visa \textquote{Linkedin} duomenų rinkinio testavimo aibe 
    slaptažodžių dalis.
  }
...
\begin{table}[hb]
  \centering
  \caption{
    Slaptažodžių, sugeneruotų \textquote{PCFG} metodo modeliu, apmokytu 80 \% 
    \textquote{RockYou} duomenų, rezultatai testuojant su 20 \% 
    \textquote{RockYou} testavimo duomenų aibe.
  }
  \begin{tabular}{|c|c|c|c|}
...
\begin{table}[hb]
  \centering
  \caption{
    Slaptažodžių, sugeneruotų \textquote{PCFG} metodu modelio, apmokytu 80 \% 
    \textquote{RockYou} duomenų, sugeneruotų slaptažodžių testavimas su visa
    \textquote{Linked} testavimo duomenų aibe.
  }
  \begin{tabular}{|c|c|c|c|}
...
\end{figure}
\ref{sec:pgpcfg} skyriuje apmokytas modelis su 80 \% \textquote{RockYou} duomenų 
rinkiniu yra toliau testuojamas su visa \textquote{Linkedin} duomenų rinkinio 
...
\textquote{PCFG} metodas ženkliai daugiau slaptažodžių parenka už 
\textquote{PassGAN} metodą -- maždaug 20-30 \% daugiau -- testuojant su 20 \% 
\textquote{RockYou} duomenų rinkinio testavimo aibe. Kitą vertus, 
\textquote{PassGAN} metodas parenka maždaug 16 \% daugiau slaptažodžių anksčiau 
nematytame \textquote{Linkedin} duomenų rinkinyje už \textquote{PCFG} metodą.
Taip pat paminėtina, kad \textquote{PCFG} metodas mažiau naujų slaptažodžių 
...
parenkamų slaptažodžių ilgių analizė bendrai \textquote{PassGAN} ir 
\textquote{PCFG} metodams (žr. \ref{plot:picked-lengths}). Analizė atlikta su 
$10^8$ generuotais slaptažodžių sąrašais \textquote{RockYou} ir 
\textquote{Linkedin} nutekintų slaptažodžių duomenų bazėms, ir pavaizduoti tik 
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{
    \textquote{PassGAN} ir \textquote{PCFG} metodų modelių parinktų slaptažodžių 
    ilgiai duomenų bazėms \textquote{RockYou} (20 \% testavimo aibė) ir 
    \textquote{Linkedin}.
  }
...
\textquote{PassGAN} ir \textquote{PCFG} -- palyginimas sėkmingai išsprendė 
...
-- su \textquote{PCFG} metodo apmokytu modeliu parenkamas aukštesnis skaičius 
slaptažodžių, panašių į apmokymui naudotus slaptažodžius, o \textquote{PassGAN} 
...
In conclusion, the experiments performed with the \textquote{PassGAN} and 
\textquote{PCFG} automated password guessing techniques have successfully completed 
...
drawbacks -- a model trained with the \textquote{PCFG} method has the ability to 
guess more passwords that are similar to those used in training, while the 
\textquote{PassGAN} model performs better guessing passwords in previously 
...
\section{\textquote{PassGAN} neuroninių tinklų sandaros} \label{add:passganarch}
\begin{figure}[H]
  \centering
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \draw[-stealth] (linear.south) -| (resblock1.north) node[midway,right] 
      {Reshape};
      ...
      \draw[-stealth] (conv1d.south) -| (softmax.north) node[]{Transpose 
      Operation};
  ...
  \begin{subfigure}[b]{0.3\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \draw[-stealth] (conv1d.south) -| (resblock1.north) node[midway,right] 
      {Reshape};
      \draw[-stealth] (resblock1.south) -| (resblock2.north) node[]{};
      \draw[-stealth] (resblock2.south) -| (resblock3.north) node[]{};
      \draw[-stealth] (resblock3.south) -| (resblock4.north) node[]{};
  ...
  \end{subfigure}
  \caption{
    \textquote{PassGAN} metodo neuroniniai tinklai.
  }
\end{figure}

\section{\textquote{PassGAN} ir \textquote{PCFG} metodų rezultatų apžvalga} 
\label{add:methods-overview}
\begin{figure}[!ht]
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[
    ...
    \caption{
      ...
      modelių, apmokytų 80 \% \textquote{RockYou} nutekintos slaptažodžių duomenų 
      bazės įrašų, testuojant su 20 \% to paties duomenų rinkinio testavimo 
      duomenų aibės.
    }
...
\begin{figure}[!ht]
  \begin{center}
    ...
    \caption{
      Parinkti slaptažodžiai tarp \textquote{PassGAN} ir \textquote{PCFG} metodo 
      modelių, apmokytų 80 \% \textquote{RockYou} nutekintos slaptažodžių 
      duomenų bazės įrašų, testuojant su viso \textquote{Linkedin} duomenų 
      rinkinio testavimo duomenų aibe.
    }
